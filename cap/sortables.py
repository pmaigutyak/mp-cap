...
class SortableListForm(ModelForm):
    ...
    class Meta:
        widgets = {
            'order': NumberInput(
                attrs={'class': 'hidden-xs-up cap-sortable'})
        }
...
class SortableStackedInlineBase(SortableModelAdminBase):
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        if db_field.name == self.sortable:
            ...
            kwargs['widget'].attrs['class'] += ' cap-sortable-stacked'
            kwargs['widget'].attrs['rowclass'] = ' cap-sortable-stacked-row'
        return super().formfield_for_dbfield(db_field, **kwargs)
