...
def get_form_size(fieldset):
    ...
    form_size_by_model_admin = getattr(fieldset.model_admin, 'cap_form_size', {})
...
def get_form_class(field, fieldset, idx):
    ...
    if not form_size:
        raise Exception('"form_size" parameter must be set in Django Cap config')
    ...
    if idx == 1:
        extra_class = ' %s' % cap_form_field_widget_class(field)
    ...
    assert isinstance(field_class, (tuple, list)) and len(field_class) == 2, \
        u'Django Cap form_size definition must be list or tuple containing two string items. ' \
...
@register.filter
def cap_form_label_class(field, fieldset):
    """
...
@register.filter
def cap_form_field_class(field, fieldset):
    """
...
@register.filter
def cap_form_field_widget_class(field):
    """
...
@register.filter
def cap_form_field_placeholder(field, placeholder):
    """
    Get CSS class for field by widget name, for easier styling
    """
