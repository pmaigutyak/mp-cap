...
from cap.apps import ALL_FIELDS
...
def result_row_attrs(context, cl, row_index):
    ...
    cap_row_attributes = getattr(cl.model_admin, 'cap_row_attributes', None)
    if not cap_row_attributes:
        return dict_to_attrs(attrs)

    instance = cl.result_list[row_index]
    new_attrs = cap_row_attributes(**{'obj': instance, 'request': context.request})
    if not new_attrs:
        return dict_to_attrs(attrs)

    # Validate
    if not isinstance(new_attrs, dict):
        raise TypeError('"cap_row_attributes" must return dict. Got: %s: %s' %
...
def headers_handler(result_headers, cl):
    ...
    cap_column_attributes = getattr(cl.model_admin, 'cap_column_attributes', None)
    if not cap_column_attributes:
        return result_headers

    for i, header in enumerate(result_headers):
        field_name = cl.list_display[i]
        attrs = copy(cap_column_attributes(field_name))
        if not attrs:
            continue

        # Validate
        if not isinstance(attrs, dict):
            raise TypeError('"cap_column_attributes" method must return dict. '
                            'Got: %s: %s' % (
...
def cells_handler(results, cl):
    ...
    """
    cap_cell_attributes = getattr(cl.model_admin, 'cap_cell_attributes', None)
    if not cap_cell_attributes:
        return results
    ...
    for row, result in enumerate(results):
        ...
        for col, item in enumerate(result):
            ...
            attrs = copy(cap_cell_attributes(instance, field_name))
            if not attrs:
                continue

            # Validate
            if not isinstance(attrs, dict):
                raise TypeError('"cap_cell_attributes" method must return dict. '
                                'Got: %s: %s' % (
                                    attrs.__class__.__name__, attrs))
...
@register.inclusion_tag('cap/search_form.html')
def cap_search_form(cl):
    """
...
@register.simple_tag
def cap_admin_list_filter(cl, spec):
