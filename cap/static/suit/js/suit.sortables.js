...
(function ($) {
    $.fn.cap_list_sortable = function () {
        ...
        $inputs.each(function () {
            var $inline_sortable = $('<div class="inline-sortable"/>'),
                ...
                $sortable = $(this),
                is_stacked = $sortable.hasClass('cap-sortable-stacked');
        ...
        function filter_unchanged(i, input) {
            ...
            } else if ($(input).hasClass('cap-sortable')) {
        ...
        if ($inputs && $inputs.length) {
            ...
            $($last_input[0].form).submit(function (e) {
                ...
                $(selector).each(function () {
                    ...
                    var $changed_fields = $set_block.find(":input[type!='hidden']:not(.cap-sortable)").filter(
                        function () {
                            return $(this).val() != "";
        ...
        Cap.after_inline.register('bind_sortable_arrows', function (prefix, row) {
            $(row).find('.cap-sortable').on('click', onArrowClick);
            markLastInline($(row).parent());
        });
    };
    ...
    $(function () {
        $('.cap-sortable').cap_list_sortable();
    });

}(django.jQuery));

// Call Cap.after_inline
django.jQuery(document).on('formset:added', function (e, row, prefix) {
    Cap.after_inline.run(prefix, row);
});
